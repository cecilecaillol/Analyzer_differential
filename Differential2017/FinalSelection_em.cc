...
int main(int argc, char** argv) {
   ...
   int nbhist=1;
   if (sample=="data_obs") nbhist=1+36;
   else if (is_higgs) nbhist=1+62+18+8; 
   else if (sample=="embedded") nbhist=1+36+16+2;
   else nbhist=1+62+36;
...
lheweight_nominal=1.0;//
	...
	for (int k=0; k<nbhist; ++k){
	   ...
	   float mass0=myvar;
           ...
           uncorrected_pth=(myele+mymu+mymet).Pt();
           pth=uncorrected_pth*fit_pthcorrection->Eval(uncorrected_pth);
           if (pth<0) pth=0.5;
